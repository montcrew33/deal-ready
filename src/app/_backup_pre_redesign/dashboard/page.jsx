...
const STATUS_META = {
  setup:      { label: 'Setup',      glow: 'bg-stone-400',   glowShadow: '',                                             text: 'text-stone-400' },
...
function ReadinessRing({ score }) {
  ...
  return (
    <svg width="130" height="130" viewBox="0 0 130 130" style={{ transform: 'rotate(-90deg)' }}>
      <circle cx="65" cy="65" r={r} fill="none" stroke="rgba(0,0,0,0.07)" strokeWidth="6" />
      <circle
    ...
    >
      ...
      <div className="px-5 pt-7 pb-8">
        <div className="flex items-center gap-2.5 text-stone-800">
          <div className="w-7 h-7 rounded-lg bg-black/6 border border-black/10 flex items-center justify-center">
            <IcShield size={13} />
          </div>
          <span className="text-[15px] font-semibold tracking-tight">DealReady</span>
        </div>
      </div>

      {/* Nav */}
      <nav className="px-3 flex-1 space-y-0.5">
        <p className="text-[10px] font-semibold tracking-widest text-stone-400 uppercase px-3 mb-3">
          Navigation
        </p>
        {navItems.map(item => (
      ...
      <div className="p-4" style={{ borderTop: '1px solid rgba(0,0,0,0.07)' }}>
        <div className="flex items-center gap-3 px-2 py-2">
          <div className="w-7 h-7 rounded-full bg-black/8 border border-black/10 flex items-center justify-center text-[11px] font-bold text-stone-600 shrink-0">
            {initial}
          </div>
          <span className="text-[12px] text-stone-400 truncate flex-1">{userEmail}</span>
          <button
            onClick={onLogout}
...
function SessionCard({ session: s }) {
  ...
  return (
    <Link href={href}>
      <div className="bg-white/55 backdrop-blur-2xl border border-white/80 rounded-3xl p-6 hover:bg-white/70 transition-colors duration-300 relative overflow-hidden group cursor-pointer h-full flex flex-col shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <h3 className="text-[15px] font-semibold text-stone-800 tracking-tight leading-snug line-clamp-2 flex-1">
            {s.company_name}
          </h3>
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          {s.industry && (
            <span className="text-[10px] text-stone-500 bg-black/5 border border-black/8 rounded-full px-2.5 py-0.5 font-medium">
              {s.industry}
            </span>
          )}
          {s.likely_buyer_type && (
            <span className="text-[10px] text-stone-500 bg-black/5 border border-black/8 rounded-full px-2.5 py-0.5 font-medium capitalize">
        ...
        {/* Divider */}
        <div className="border-t border-black/6 pt-4 mt-auto">
          {/* Progress */}
          <div className="mb-3.5">
            <div className="h-[1.5px] bg-black/8 rounded-full overflow-hidden">
              <div
          ...
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-stone-400">{formatDate(s.created_at)}</span>
            <span className="text-[11px] font-semibold text-stone-400 group-hover:text-stone-700 transition-colors flex items-center gap-1">
...
function EmptyState() {
  return (
    <div className="bg-white/55 backdrop-blur-2xl border border-white/80 rounded-3xl p-16 text-center col-span-3 shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
      <div className="w-12 h-12 rounded-2xl bg-black/5 border border-black/8 flex items-center justify-center mx-auto mb-5 text-stone-400">
        <IcShield size={20} />
      </div>
      <h2 className="text-[15px] font-semibold text-stone-700 tracking-tight mb-2">No sessions yet</h2>
      <p className="text-[13px] text-stone-400 max-w-[260px] mx-auto leading-relaxed mb-8">
        Create your first session to start preparing for buyer management presentations.
      </p>
      <Link href="/dashboard/new">
        <button className="bg-stone-900 text-white text-[13px] font-semibold px-6 py-2.5 rounded-full hover:scale-105 transition-transform duration-300 shadow-[0_4px_16px_rgba(0,0,0,0.18)] inline-flex items-center gap-2">
          <IcPlus /> New Session
        </button>
      </Link>
    </div>
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center"
        style={{ background: 'linear-gradient(135deg, #F5EDE0 0%, #EDE2CC 55%, #F0E8D8 100%)' }}>
        <div className="w-5 h-5 rounded-full border border-stone-400/40 border-t-stone-600 animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div
      ...
      style={{
        ...
        background: 'linear-gradient(135deg, #F5EDE0 0%, #EDE2CC 55%, #F0E8D8 100%)',
      }}
    >
      {/* ── Ambient background glows (warm tones) ───────────────────────────── */}
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        ...
        <div style={{
          ...
          background: 'radial-gradient(circle, rgba(214,122,95,0.09) 0%, transparent 65%)',
          filter: 'blur(40px)',
        }} />
      </div>
      ...
      <main className="flex-1 overflow-y-auto relative z-10 min-w-0">
        <div className="max-w-5xl mx-auto px-8 py-10">
          ...
          <div className="flex items-end justify-between mb-10">
            ...
            <Link href="/dashboard/new">
              <button className="bg-stone-900 text-white text-[13px] font-semibold px-6 py-2.5 rounded-full hover:scale-105 transition-transform duration-300 shadow-[0_4px_20px_rgba(0,0,0,0.20)] flex items-center gap-2 shrink-0">
                <IcPlus /> New Session
              </button>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            ...
            <div className="col-span-2 bg-white/55 backdrop-blur-2xl border border-white/80 rounded-3xl p-7 shadow-[0_4px_24px_rgba(0,0,0,0.06)] relative overflow-hidden">
              <div className="absolute inset-0 rounded-3xl"
                style={{ background: 'radial-gradient(ellipse at 80% 20%, rgba(245,158,11,0.07) 0%, transparent 60%)' }} />
              <p className="text-[10px] font-semibold tracking-widest text-stone-400 uppercase mb-5">
                M&amp;A Readiness Score
              </p>
              <div className="flex items-center gap-7">
                ...
                <div className="relative shrink-0">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-3xl font-semibold tracking-tight text-stone-800 leading-none">{readiness}%</span>
                    <span className="text-[9px] tracking-widest text-stone-400 uppercase mt-1">Ready</span>
                  </div>
                </div>
                {/* Text */}
                <div>
                  <h2 className="text-xl font-semibold tracking-tight text-stone-700 mb-2">
                  ...
                  <div className="flex items-center gap-4 mt-5">
                    <div className="text-center">
                      <p className="text-2xl font-semibold tracking-tight text-stone-700">{sessions.length}</p>
                      <p className="text-[10px] tracking-widest text-stone-400 uppercase">Sessions</p>
                    </div>
                    <div className="w-px h-8 bg-black/8" />
                    <div className="text-center">
                      <p className="text-2xl font-semibold tracking-tight text-emerald-500">{complete}</p>
                      <p className="text-[10px] tracking-widest text-stone-400 uppercase">Complete</p>
                    </div>
                    <div className="w-px h-8 bg-black/8" />
                    <div className="text-center">
                      <p className="text-2xl font-semibold tracking-tight text-indigo-500">{inProgress}</p>
                      <p className="text-[10px] tracking-widest text-stone-400 uppercase">Active</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {/* Blueprint card */}
            <div className="bg-white/55 backdrop-blur-2xl border border-white/80 rounded-3xl p-6 shadow-[0_4px_24px_rgba(0,0,0,0.06)] flex flex-col justify-between relative overflow-hidden">
              <div className="absolute inset-0 rounded-3xl"
                style={{ background: 'radial-gradient(ellipse at 20% 80%, rgba(167,139,250,0.08) 0%, transparent 60%)' }} />
              <div>
                <div className="w-9 h-9 rounded-xl bg-black/6 border border-black/8 flex items-center justify-center text-stone-500 mb-4">
                  <IcBookOpen />
                </div>
                <p className="text-[10px] font-semibold tracking-widest text-stone-400 uppercase mb-2">Resource</p>
                <h3 className="text-[15px] font-semibold text-stone-700 tracking-tight mb-2">
              ...
              <div className="border-t border-black/6 pt-4 flex items-center justify-between">
                <span className="text-[10px] font-semibold tracking-widest text-stone-400 uppercase">Recommended</span>
                <button className="text-[11px] font-semibold text-stone-400 hover:text-stone-700 transition-colors flex items-center gap-1">
                  Open <IcChevron />
                </button>
              </div>
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-5">
              ...
              {sessions.length > 0 && (
                <span className="text-[11px] text-stone-400">{sessions.length} total</span>
              )}
            </div>
